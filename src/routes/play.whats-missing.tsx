...
function WhatsMissing() {
  ...
  const set = useMemo(() => [...ITEMS].sort(() => Math.random() - 0.5).slice(0, showCount), [round, showCount]);
  const missing = useMemo(() => set[Math.floor(Math.random() * set.length)], [set]);
  const shown = useMemo(() => set.filter((s) => s !== missing).sort(() => Math.random() - 0.5), [set, missing]);
    ...
    return (
      <GameShell title="What's Missing?" instructions="">
        <ResultPanel
          ...
          total={1}
          startedAt={startedAt}
          onAgain={() => { setDone(false); setCorrect(0); setPick(null); setPhase("study"); setRound((r) => r + 1); }}
        />
      </GameShell>
    );
  }
  ...
  return (
    <GameShell
      ...
      instructions={phase === "study" ? "Look carefully at all the items." : "One item from before is gone. Which one?"}
    >
      <div className="card-soft p-6">
        <div className="grid grid-cols-4 gap-4">
          {(phase === "study" ? set : shown).map((s, i) => (
            <div key={i} className="aspect-square grid place-items-center text-5xl sm:text-6xl rounded-2xl bg-secondary">
              {s}
            </div>
          ))}
        </div>
          ...
          {options.map((o) => (
            <button
              ...
              className={[
                ...
                pick === o && o === missing ? "bg-success text-success-foreground" :
                pick === o ? "bg-destructive/30" :
                pick && o === missing ? "bg-success/40" :
