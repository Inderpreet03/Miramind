...
function FamilyFaces() {
  ...
  const options = useMemo(() => {
    if (!current) return [];
    const others = family.filter((f) => f.id !== current.id).sort(() => Math.random() - 0.5).slice(0, 3);
    return [current, ...others].sort(() => Math.random() - 0.5);
  }, [current, family]);

  if (family.length < 2) {
    return (
      <GameShell title="Family Faces" instructions="Add at least 2 family members in the Family hub to start this game.">
    ...
    return (
      <GameShell title="Family Faces" instructions="">
        <ResultPanel
          ...
          total={order.length}
          startedAt={startedAt}
          onAgain={() => { setI(0); setCorrect(0); setPick(null); setRound((r) => r + 1); }}
        />
      </GameShell>
    );
  }
  ...
  function choose(name: string) {
    ...
    setTimeout(() => { setPick(null); setI((x) => x + 1); }, 1000);
  }
  ...
  return (
    <GameShell title="Family Faces" instructions="Who is this?">
      <div className="card-soft p-10 text-center">
        ...
        {current.note && (
          <div className="mt-3 text-sm text-muted-foreground italic max-w-md mx-auto">"{current.note}"</div>
        )}
      </div>
        ...
        {options.map((o) => (
          <button
            ...
            className={[
              ...
              pick === o.name && o.name === current.name ? "bg-success text-success-foreground" :
              pick === o.name ? "bg-destructive/30" :
              pick && o.name === current.name ? "bg-success/40" :
      ...
      <div className="mt-4 text-center text-muted-foreground">{i + 1} / {order.length}</div>
    </GameShell>
  );
}
