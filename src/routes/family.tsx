...
export const Route = createFileRoute("/family")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Add loved ones, send personalised tasks and voice messages, follow progress from anywhere." },
    ],
  }),
  component: FamilyHub,
});
...
function FamilyHub() {
  ...
  const avgAcc = last7.length ? Math.round((last7.reduce((a, s) => a + s.accuracy, 0) / last7.length) * 100) : 0;

  return (
    <main className="mx-auto max-w-6xl px-4 pt-10 pb-24">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Family hub</p>
          <h1 className="font-display text-4xl sm:text-5xl mt-1">Be there, every day.</h1>
          <p className="mt-2 text-muted-foreground max-w-2xl">
            Send {resident.name} a small task, a photo or a voice note. Watch progress without
            being intrusive.
          </p>
        </div>
      ...
      <section className="mt-12">
        <div className="flex items-end justify-between">
          <h2 className="font-display text-2xl flex items-center gap-2"><Users className="size-5" /> Loved ones</h2>
        </div>
          ...
          {tasks.slice(0, 8).map((t) => (
            <li key={t.id} className="card-soft px-4 py-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <span className={`size-2.5 rounded-full ${t.completedAt ? "bg-success" : "bg-warning"}`} />
                <div>
...
function SendTaskCard() {
  ...
  function submit(e: React.FormEvent) {
    ...
    store.addTask({ kind, title: title.trim(), detail: detail.trim() || undefined, assignedBy: from });
    setTitle(""); setDetail("");
  }
  ...
  return (
    <form onSubmit={submit} className="card-soft p-6">
      ...
      <div className="mt-5 grid sm:grid-cols-2 gap-3">
        ...
        <Field label="Type">
          <select value={kind} onChange={(e) => setKind(e.target.value as TaskKind)} className="input">
      ...
      <Field label="Title">
        <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. A morning hello from Lukas" className="input" />
      </Field>
      <Field label="Note (optional)">
        <textarea value={detail} onChange={(e) => setDetail(e.target.value)} placeholder="Add a message or context..." className="input min-h-24" />
      </Field>
...
function AddMemberCard() {
  ...
  function add(e: React.FormEvent) {
    e.preventDefault();
    if (!name.trim()) return;
    store.addFamily({ name: name.trim(), relation: relation.trim() || "Family", note: note.trim() || undefined, emoji });
    setName(""); setRelation(""); setNote(""); setOpen(false);
  }
  ...
  return (
    <form onSubmit={add} className="card-soft p-5">
      ...
      <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Name" className="mt-3 w-full px-3 py-2 rounded-lg border border-border bg-card" />
      <input value={relation} onChange={(e) => setRelation(e.target.value)} placeholder="Relation (Daughter, Grandson...)" className="mt-2 w-full px-3 py-2 rounded-lg border border-border bg-card" />
      <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="A small memory cue..." className="mt-2 w-full px-3 py-2 rounded-lg border border-border bg-card" />
      <div className="mt-3 flex gap-2">
        <button type="submit" className="flex-1 btn-large py-2 text-base bg-primary text-primary-foreground">Save</button>
        <button type="button" onClick={() => setOpen(false)} className="btn-large py-2 text-base bg-card border border-border">Cancel</button>
      </div>
    </form>
  );
}
