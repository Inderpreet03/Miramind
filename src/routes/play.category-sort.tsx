...
function CategorySort() {
  ...
  const items = useMemo(() => [...POOL].sort(() => Math.random() - 0.5).slice(0, total), [round, total]);
    ...
    return (
      <GameShell title="Category Sort" instructions="">
        <ResultPanel
          ...
          total={items.length}
          startedAt={startedAt}
          onAgain={() => { setIdx(0); setCorrect(0); setFeedback(null); setRound((r) => r + 1); }}
        />
      </GameShell>
    );
  }
  ...
  return (
    <GameShell title="Category Sort" instructions="Which group does this belong to?">
        ...
        {cats.map((c) => (
          <button
            ...
            className={[
              ...
              feedback === "ok" && c === current.cat ? "bg-success text-success-foreground" :
              feedback === "no" && c === current.cat ? "bg-success/40" :
      ...
      <div className="mt-4 text-center text-muted-foreground">{idx + 1} / {items.length}</div>
    </GameShell>
  );
}
