...
function SequenceGame() {
  ...
  const total = 5;
  const questions = useMemo(() => Array.from({ length: total }, () => makeQ(difficulty)), [round, difficulty]);
  ...
  function choose(n: number) {
    ...
    setTimeout(() => { setPick(null); setI((x) => x + 1); }, 800);
  }
  ...
  return (
    <GameShell title="Sequence" instructions="Which number is missing?">
          ...
          {q.seq.map((v, idx) => (
            <div
              ...
              className={[
                ...
                v === null ? "bg-accent/20 text-accent border-2 border-dashed border-accent" : "bg-secondary text-secondary-foreground",
              ].join(" ")}
            >
              {v ?? "?"}
            </div>
        ...
        {q.options.map((o) => (
          <button
            ...
            className={[
              ...
              pick === o && o === q.answer ? "bg-success text-success-foreground" :
              pick === o ? "bg-destructive/30" :
              pick !== null && o === q.answer ? "bg-success/40" :
      ...
      <div className="mt-4 text-center text-muted-foreground">{i + 1} / {questions.length}</div>
    </GameShell>
  );
}
