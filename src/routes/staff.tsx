...
export const Route = createFileRoute("/staff")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Track residents' cognitive sessions, adapt difficulty, export progress reports." },
    ],
  }),
...
function StaffDashboard() {
  ...
  const avgAcc = mounted && sessions.length
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 pt-10 pb-24">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Staff dashboard</p>
          <h1 className="font-display text-4xl sm:text-5xl mt-1">Sonnenhof Pflegeheim</h1>
          <p className="mt-2 text-muted-foreground">3 residents in pilot · adaptive difficulty engine v1</p>
        </div>
        <button
          onClick={() => store.reset()}
      ...
      <section className="mt-10 grid lg:grid-cols-[1.4fr_1fr] gap-6">
        <div className="card-soft p-6">
          ...
          <table className="mt-4 w-full text-sm">
            ...
            <tbody>
              <Row name={resident.name} difficulty={resident.difficulty} streak={resident.streakDays} sessions={totalSessions} accent />
        ...
        <div className="card-soft p-6">
          ...
          {recent.length === 0 ? (
            <p className="mt-4 text-muted-foreground">No sessions yet — play a few games to see live data.</p>
          ) : (
            <ul className="mt-4 space-y-2">
              {recent.map((s) => (
                <li key={s.id} className="flex items-center justify-between rounded-xl bg-secondary/60 px-3 py-2">
                  <div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-muted-foreground">{Math.round(s.avgResponseMs / 100) / 10}s avg</div>
                  </div>
                </li>
              ))}
            </ul>
      ...
      <section className="mt-10 card-soft p-6">
        <h2 className="font-display text-2xl">Smart difficulty rules</h2>
        <div className="mt-4 grid sm:grid-cols-3 gap-4 text-sm">
          <Rule color="bg-success/20 text-success-foreground" title="Increase">accuracy &gt; 80% AND response &lt; 5s</Rule>
          <Rule color="bg-warning/30 text-warning-foreground" title="Stay">balanced performance</Rule>
          <Rule color="bg-destructive/15 text-destructive" title="Decrease">accuracy &lt; 50% OR response &gt; 15s</Rule>
        </div>
      </section>
    </main>
  );
...
function Row({ name, difficulty, streak, sessions, accent }: { name: string; difficulty: number; streak: number; sessions: number; accent?: boolean }) {
  return (
    <tr className="border-t border-border">
      <td className="py-3 font-semibold">
        {name} {accent && <span className="ml-2 text-xs px-2 py-0.5 rounded-full bg-accent/20 text-accent">demo</span>}
      </td>
