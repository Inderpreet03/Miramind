// Shared scaffold for game pages — provides "exit", instructions and result handling.
...
export function GameShell({
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 pt-8 pb-24">
      <Link to="/play" className="inline-flex items-center gap-2 text-muted-foreground hover:text-foreground">
...
export function ResultPanel({
  ...
  const [saved] = useState(() => {
    store.recordSession({ kind, accuracy, avgResponseMs: avg, difficulty: store.get().resident.difficulty, durationMs: duration });
    return true;
  });
  void saved;
  const message =
    accuracy >= 0.9 ? "Wonderful work!" : accuracy >= 0.6 ? "Nicely done." : "Well tried — every game helps.";
  return (
    <div className="card-soft p-8 text-center">
      ...
      <div className="mt-6 flex gap-3 justify-center">
        ...
        <Link to="/play" className="btn-large bg-card border border-border">Back</Link>
      </div>
    </div>
  );
}
